...
export default function OnboardingPage() {
  ...
  async function handleVerifyCode(e: React.FormEvent) {
    ...
    if (result.error === 'invalid') {
      setError('Code ungültig. Wende dich an deinen Account Manager.')
      return
    }
    if (result.error === 'already_completed') {
      setError('Dieses Konto wurde bereits eingerichtet. Bitte melde dich an.')
      return
    }
    if (result.error) {
  ...
  async function handleCreateAccount(e: React.FormEvent) {
    ...
    if (signInErr) {
      setError(`Konto erstellt, aber Anmeldung fehlgeschlagen: ${signInErr.message}`)
      setLoading(false)
      return
    }
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-3xl shadow-sm border border-brand-pink/20 p-10">
          <div className="text-center mb-8">
            ...
            {step === 1 ? (
              <>
                <h1 className="font-playfair text-3xl text-brand-green leading-tight">Willkommen 🌸</h1>
              ...
              <>
                <h1 className="font-playfair text-3xl text-brand-green leading-tight">Willkommen beim Papaya Social Club 🌸</h1>
                {creator?.name && (
