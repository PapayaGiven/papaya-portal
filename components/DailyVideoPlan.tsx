    ...
    return (
      <div className="bg-white border border-gray-100 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
          <h3 className="font-playfair text-2xl text-brand-black">Dein Plan für heute</h3>
          <p className="font-dm-sans text-xs text-gray-400 capitalize">{todayLabel}</p>
        </div>
        <p className="font-dm-sans text-sm text-gray-500">
          Deine Strategie ist bald bereit. Dein Account Manager bereitet sie gerade vor.
        </p>
      </div>
    )
  }
    ...
    <div className="bg-white border border-gray-100 rounded-2xl p-6">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        ...
        <span className="font-dm-sans text-xs font-semibold bg-brand-green/10 text-brand-green px-2.5 py-1 rounded-full">
          {totalDone} von {totalTarget} Videos
        </span>
      </div>
                ...
                {Array.from({ length: target }).map((_, i) => {
                  ...
                  return (
                    <button
                      ...
                      aria-label={checked ? `Video ${i + 1} veröffentlicht` : `Video ${i + 1} markieren`}
      ...
      <div className="mt-5 pt-4 border-t border-gray-50 flex items-center justify-between flex-wrap gap-2">
        <p className="font-dm-sans text-xs text-gray-500">
          {totalDone === totalTarget && totalTarget > 0
            ? '🎉 Tagesplan erledigt!'
            : `${totalTarget - totalDone} Video${totalTarget - totalDone === 1 ? '' : 's'} offen.`}
        </p>
...
function VideoLogModal({
  ...
  function submit() {
    if (!tiktokUrl.trim()) {
      setError('Bitte den TikTok-Link einfügen.')
      return
    }
    setError(null)
    ...
    >
      <div className="bg-white rounded-3xl border border-gray-100 w-full max-w-md p-6 shadow-2xl">
        <div className="text-center mb-5">
          ...
          <h3 className="font-playfair text-2xl text-brand-black">Gut gemacht!</h3>
          <p className="font-dm-sans text-sm text-gray-500 mt-1">Füge den Link zu deinem Video hinzu.</p>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="block font-dm-sans text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
              Spark-Ads-Code <span className="text-gray-300 font-normal">(optional)</span>
            </label>
            <input
              type="text"
              value={sparkCode}
        ...
        <div className="flex gap-2 mt-5">
          ...
          >
            {isPending ? 'Speichern…' : 'Video speichern'}
          </button>
        </div>
      </div>
    </div>
