    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-5">
      <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
        <h3 className="font-dm-sans font-semibold text-brand-black text-sm">
          Videos heute: <span className="font-bold">{done}</span> von <span className="font-bold">{safeTarget}</span>
        </h3>
        {isDone && (
          <span className="font-dm-sans text-xs font-semibold text-brand-green">🎉 Tagesziel erreicht!</span>
        )}
      </div>
      ...
      <p className="font-dm-sans text-xs text-gray-500 mt-2">
        {Math.round(pct)}% deines Tagesziels
      </p>
    </div>
  )
}
